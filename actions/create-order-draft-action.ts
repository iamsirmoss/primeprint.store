...
//       const unitPriceCents = toCents(p.basePriceCents / 100);
...
//       const price = it.billing === "month" ? pkg.priceMonthCents : pkg.priceYearCents;
//       if (!price || price <= 0) throw new Error("Package price invalid.");
...
export async function createOrderDraftAction(payload: Payload) {
  ...
  for (const it of payload.items) {
    ...
    if (it.type === "PRODUCT") {
      ...
      if (available < requested) {
        throw new Error(`"${p.title}" has only ${available} left (you requested ${requested}).`);
      }

      currency = p.currency ?? "USD";

      const unitPriceCents = p.basePriceCents;
    ...
    if (it.type === "PACKAGE") {
      ...
      const unitPriceCents = price;
      const totalPriceCents = unitPriceCents * qty;

      subtotalCents += totalPriceCents;

      const billingLabel = it.billing === "month" ? "Monthly" : "Yearly";
      const serviceTitle = pkg.subService?.service?.title ?? pkg.service?.title ?? null;
      const subServiceTitle = pkg.subService?.title ?? null;
