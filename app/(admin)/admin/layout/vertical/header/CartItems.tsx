...
const CartItems = () => {
  ...
  const handleQuantity = (productType: string, actionType: string) => {
    if (productType === "product1") {
      SetProdQuant1(actionType === "inc" ? prodQuant1 + 1 : Math.max(0, prodQuant1 - 1));
    } else if (productType === "product2") {
      SetProdQuant2(actionType === "inc" ? prodQuant2 + 1 : Math.max(0, prodQuant2 - 1));
    } else {
      SetProdQuant3(actionType === "inc" ? prodQuant3 + 1 : Math.max(0, prodQuant3 - 1));
    }
  };
  ...
  return (
    <>
      <button
        ...
        className="hover:text-primary relative after:absolute after:w-10 after:h-10 after:rounded-full after:-top-2.5 hover:after:bg-lightprimary flex justify-center items-center cursor-pointer"
      >
          ...
          <SimpleBar className="h-[calc(100vh-85px)]">
            ...
            <div className="px-6">
              ...
              <Button variant="outline" className="w-full">
                <Link href="/auth/auth1/login">Go to shopping cart</Link>
              </Button>
            </div>
          </SimpleBar>
        </SheetContent>
